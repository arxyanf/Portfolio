...
const TimelineNode = ({ isActive, type }: TimelineNodeProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Main node */}
      <div className={`relative w-11 h-11 md:w-12 md:h-12 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
        isActive 
...
const TimelineCard = ({ item, index, totalItems }: TimelineCardProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`relative rounded-xl overflow-hidden bg-card/60 border transition-all duration-300 ${
          isInView ? 'border-primary/30' : 'border-border/30'
        }`}>
